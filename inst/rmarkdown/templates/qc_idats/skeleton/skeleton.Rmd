...
stopifnot(all(sapply(
  X = c(
    "CARoT", "doParallel", "dplyr", "ENmix", "ggplot2", "grid", 
    "Hmisc", "kableExtra", "knitr", "minfi", "parallel", "parallel", 
    "rain", "readr", "scales", "sessioninfo", "stats", "tibble", 
    "tidyr", "utils", "magrittr"
  ),
  FUN = require,
  character.only = TRUE,
...
if (!requireNamespace("FlowSorted.Blood.EPIC")) {
...
if (
  tolower(params[["cell_tissue"]]) %in% "blood" & 
    !requireNamespace(paste0("FlowSorted.Blood.", params[["array"]]))
) {
  do_cell_composition <- FALSE
...
if (
  !tolower(params[["cell_tissue"]]) %in% c("blood", "cordblood") & 
    !requireNamespace("RefFreeEWAS")
) {
  do_cell_composition <- FALSE
...
cell_comp <- switch(
  ...
  "cordblood" = {
    if (requireNamespace("FlowSorted.CordBloodCombined.450k")) {
  ...
  },
  {
    ...
    beta_matrix <- minfi::getBeta(data_rgset)
...
ggplot2::ggplot(
  ...
  ggplot2::geom_abline(
    data = data.frame(Threshold = paste("=", gender_threshold), Seuil = -1), 
...
```{r export}
readr::write_rds(x = data_mset, path = file.path(output_directory, paste0(params[["array"]], "_QC_mset.rds")))
readr::write_csv(as.data.frame(norm_beta), path = file.path(output_directory, paste0(params[["array"]], "_QC_betavalues.csv")))
readr::write_csv(as.data.frame(phenotypes), path = file.path(output_directory, paste0(params[["array"]], "_QC_phenotypes.csv")))
```
